...
if __name__ == '__main__':
    ...
    URL = "https://www.safeway.com/Fresh-Fruits/b?ie=UTF8&node=16318981"
    ...
    for page in range(1, 27):  # Looping through all 26 pages
            ...
            URL = f"https://www.safewa.com/s?k=Fresh+Fruits&i=grocery&rh=n%3A16318981&page={page}&c=ts&qid=1731997546&ts_id=16318981&ref=sr_pg_{page}"
            webpage = requests.get(URL, headers=HEADERS)
            soup = BeautifulSoup(webpage.content, "html.parser")
            ...
            for link in links_list:
                if link.startswith("/"):
                    new_webpage = requests.get(f"https://safeway.com{link}", headers=HEADERS)
                else:
    ...
    df = pd.DataFrame(d)
    df.to_csv("safeway_products.csv", index=False)
    print("Scraping completed and data saved to safeway_products.csv")
